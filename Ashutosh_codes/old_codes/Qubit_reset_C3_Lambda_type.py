...
"""
swap_params = {
    "amp": Qty(value=0.1,min_val=0.1,max_val=10.0,unit="V"),
...
swap_params = {
    "amp": Qty(value=0.5,min_val=0.2,max_val=1.0,unit="V"),
    "t_final": Qty(value=t_swap_gate,min_val=10e-9,max_val=300e-9,unit="s"),
...
swap_pulse = pulse.Envelope(
    name="swap_pulse",
    desc="Gaussian pluse for SWAP gate",
    params=swap_params,
    shape=envelopes.gaussian_nonorm #envelopes.flattop
)
...
pwc_params = {
    ...
    "t_bin_end": Qty(value=t_swap_gate, min_val=5e-9, max_val=200e-9, unit="s"),
    "inphase": Qty(value=swap_gate_shape, min_val=0.0, max_val=1.0, unit="") 
}

pwc_swap_pulse = pulse.Envelope(
    name="swap_pulse_pwc",
    desc="PWC pluse for SWAP gate",
    params=pwc_params,
...
index = model.get_state_indeces([(1,0),(1,1)])
...
sequence = ['swap[0, 1]']
states_to_plot = [(0,1), (1,0), (1,1), (0,2), (2,0)]
plotPopulation(exp=exp, psi_init=init_state, sequence=sequence, states_to_plot=states_to_plot, usePlotly=False, filename="Before_optimisation_11.png")
plotPopulation(exp=exp, psi_init=init_state, sequence=sequence, usePlotly=True, filename="Before_optimisation_11")
...
parameter_map.set_opt_map([
    [("swap[0, 1]", "dR", "carrier", "freq")],
    [("swap[0, 1]", "dR", "swap_pulse_pwc", "t_bin_start")],
    [("swap[0, 1]", "dR", "swap_pulse_pwc", "t_bin_end")],
    [("swap[0, 1]", "dR", "swap_pulse_pwc", "inphase")],
    [("swap[0, 1]", "dQ", "carrier", "freq")],
    [("swap[0, 1]", "dQ", "swap_pulse_pwc", "t_bin_start")],
    [("swap[0, 1]", "dQ", "swap_pulse_pwc", "t_bin_end")],
    [("swap[0, 1]", "dQ", "swap_pulse_pwc", "inphase")],
])
...
# %%
plotPopulation(exp=exp, psi_init=init_state, sequence=sequence, usePlotly=False, filename="After_optimization_1.png")
plotPopulation(exp=exp, psi_init=init_state, sequence=sequence, usePlotly=False, states_to_plot=states_to_plot, filename="After_optimization_selected_11.png")
