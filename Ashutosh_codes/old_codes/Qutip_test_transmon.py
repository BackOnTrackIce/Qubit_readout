...
class system_setup:
    def __init__(self, qubit_freq, qubit_anharm, res_freq, drive_freq, drive_amp, g, kappa, mu1, sigma):
        self.wr = res_freq
        self.wa = qubit_freq
        self.alpha = qubit_anharm
        self.g = g
        self.kappa = kappa
        self.wd = drive_freq
...
class readout_setup:
    def __init__(self, Nr, Nq, cavity_photon_number, tlist, parameters, H_flag1, H_flag2, SW_flag1, SW_flag2, runs_flag):
        ...
        self.aR =  tensor(destroy(self.Nr),qeye(self.Nq))
        self.aQ = tensor(qeye(self.Nr), destroy(self.Nq))
        ...
        self.psi_up = tensor(basis(self.Nr,self.cavity_photon_number),basis(self.Nq,1))
        self.psi_down = tensor(basis(self.Nr,self.cavity_photon_number),basis(self.Nq,0))

        self.tlist = tlist

        self.pi_e = basis(self.Nq,1) * basis(self.Nq,1).dag()
        self.Nq_oper = self.aR.dag() * self.aR + tensor(qeye(Nr),self.pi_e)

    def JC_hamiltonian(self):
        return self.wr*self.aR.dag()*self.aR + self.wa * self.aQ.dag() * self.aQ + (self.alpha/2)*(self.aQ.dag()*self.aQ - self.I)*self.aQ.dag()*self.aQ + self.g * (self.aR.dag() * self.aQ + self.aR * self.aQ.dag())
    ...
    def solve_master_equation(self, H_flag, psi_SW_flag):
        ...
        #c_ops = [np.sqrt(self.kappa) * self.aR]
        c_ops = []
        e_ops = []
    ...
    def SW_unitary(self):
        exponent = self.lamda * (self.aR * self.aQ.dag() - self.aR.dag()*self.aQ )
        U = exponent.expm()
        return U
    ...
    def calc_dynamics(self, output, aR, aQ):
        state = output.states
        dynamics_res = [expect(aR.dag() * aR,i) for i in state]
        dynamics_qubit = [expect(aQ.dag() * aQ,i) for i in state]
        return dynamics_res, dynamics_qubit

    def plot_dynamics(self, output1, output2):
        if self.SW_flag1 == True:
            dynamics1_res, dynamics1_qubit = self.calc_dynamics(output1, self.aR.transform(self.SW_unitary()), self.aQ.transform(self.SW_unitary()))
        else:
            dynamics1_res, dynamics1_qubit = self.calc_dynamics(output1, self.aR, self.aQ)
        
        if self.SW_flag2 == True:
            dynamics2_res, dynamics2_qubit = self.calc_dynamics(output2, self.aR.transform(self.SW_unitary()), self.aQ.transform(self.SW_unitary()))
        else:
            dynamics2_res, dynamics2_qubit = self.calc_dynamics(output2, self.aR, self.aQ)
    ...
    def frame_of_drive_dm(self, output, SW_flag):
        state = output.states
        
        if SW_flag == True:
            a1 = self.aR.transform(self.SW_unitary())
            a2 = self.aQ.transform(self.SW_unitary())
        else:
            a1 = self.aR
            a2 = self.aQ

        U_exp = [1j * t * self.wd * (a1.dag() * a1 + a2.dag() * a2 ) for t in self.tlist]
    ...
    def frame_of_drive(self, output, SW_flag):
        ...
        U_exp = [1j * t * self.wd * (a1.dag() * a1 + a2.dag() * a2 + (self.alpha/2)*(a2.dag()*a2 - self.I)*a2.dag()*a2) for t in self.tlist]

        U_matrix = [i.expm() for i in U_exp]

        state_rot = [U_matrix[i] * state[i]  for i in range(len(self.tlist))]
    ...
    def plotStatePopulation(self):
        ...
        dynamics_res, dynamics_qubit = self.calc_dynamics(self.output, self.aR, self.aQ)
...
Test1 = system_setup(
                     ...
                     qubit_anharm=2 * np.pi*(-200e6),
                     res_freq = 2 * np.pi * 4 * 1e9,
                     drive_freq = 2 * np.pi * (4-0.001) * 1e9,
